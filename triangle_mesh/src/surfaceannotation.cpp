...
std::vector<std::shared_ptr<Triangle> > SurfaceAnnotation::regionGrowing(std::vector<std::vector<std::shared_ptr<Vertex> > > contours)
{
    /* Value that says when an edge is on the boundary (if the value in its "info" field is equal to BOUNDARY_EDGE,
        ...
        for(unsigned int i = 0; i < contours.size(); i++){
            ...
            t->addInformation(static_cast<void*>(&ALREADY_USED));
            ...
            for(unsigned int i = 1; i <= boundary.size() - 1; i++){
                ...
                if(e != nullptr)
                    e->addInformation(static_cast<void*>(&BOUNDARY_EDGE));
                else {
                    ...
                    std::cerr << "Starting vertex: (" << v1->getX() << "," << v1->getY() << "," << v1->getZ() << ")" << std::endl;
                    ...
                    for(unsigned int i = 0; i < ve1.size(); i++)
                    {
                        ...
                        std::cerr << counter + 1 << "-th neighbour: (" << v_->getX() << "," << v_->getY() << "," << v_->getZ() << ")" << std::endl;
                        counter++;
                    }

                    std::cerr << "Wanted vertex: (" << v2->getX() << "," << v2->getY() << "," << v2->getZ() << ")" << std::endl;
                    counter = 0;
                    std::vector<std::shared_ptr<Edge> > ve2 = v1->getVE();
        ...
        while(neighbors.size() > 0){
            ...
            for(int i = 0; i < 3; i++){
                ...
                if(e->searchInfo(&BOUNDARY_EDGE)){
                    ...
                    if(t_->searchInformation(&ALREADY_USED)){
                        internalTriangles.push_back(t_);
                        t_->addInformation(static_cast<void*>(&ALREADY_USED));
                        neighbors.push(t_);
                    }
                }
    ...
    if(outlines.size() == 0){
        ...
        for(unsigned int i = 0; i < triangles.size(); i++){
            ...
            for(unsigned int j = 0; j < 3; j++){
                ...
                if(v->getInfo() == nullptr || (*static_cast<unsigned int*>(v->getInfo()) != ALREADY_USED && *static_cast<unsigned int*>(v->getInfo()) != BOUNDARY_VERTEX)){
                    vertices.push_back(v);
                    v->setInfo(&ALREADY_USED);
                }
            }
        }
